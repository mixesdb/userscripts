...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 * Track page loading skeleton (new Material layout)
 *
 * Everything we put into #mdb-sc-trackExtras arrives asynchronously and each piece used to
 * pop in on its own: buttons/dates/artwork once the SC API answers, the toolkit once the
 * MixesDB search answers, the page creator row once the toolkit verdict is read, the
 * tracklist box after that. scSkeleton_show() covers that build-up with pulsing grey
 * stand-ins shaped like the content to come (the .mdb-sc-loading class in script.css hides
 * every other wrapper child while it is up) and scSkeleton_reveal() swaps skeleton and
 * content in ONE step, so the page goes straight from placeholder to complete.
 *
 * Revealed when the buttons AND the toolkit verdict are in the DOM and nothing has changed
 * inside the wrapper for scSkeleton_settleMs - the settle window is what lets the page
 * creator row and a description tracklist (which follow the verdict within a
 * waitForKeyElements poll or two) slip in before the swap. The API failure note counts as
 * the API being done, so a dead API does not hold the page. scSkeleton_maxMs caps the whole
 * wait: after that, whatever has arrived is shown as-is (a comments-fetched tracklist may
 * then still pop in late - rare, and it sits at the bottom).
 *
 * SPA navigation and SoundCloud's React wiping the wrapper both simply remove skeleton and
 * wrapper together; the Track header handler recreates the wrapper and calls
 * scSkeleton_show() again, which is why it starts by clearing the previous run's timers.
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

var scSkeleton_checkTimer = null,
    scSkeleton_capTimer = null,
    scSkeleton_observer = null,
    scSkeleton_lastMutation = 0,
    scSkeleton_shownAt = 0;

var scSkeleton_settleMs = 600,
    scSkeleton_maxMs = 6000;

// scSkeleton_html
// The bars mirror the real layout so the swap lands where the grey stood: headline with the
// artwork bar right of it, the dates row, the three button pills, the toolkit line and the
// tracklist block. Sizes live in script.css.
function scSkeleton_html() {
    function bars( n ) {
        var out = "", i;
        for( i = 0; i < n; i++ ) {
            out += '<div class="mdb-sc-skeleton-bar"></div>';
        }
        return out;
    }

    return '<div id="mdb-sc-skeleton">'
         +     '<div class="mdb-sc-skeleton-head">'
         +         '<div class="mdb-sc-skeleton-bar mdb-sc-skeleton-headline"></div>'
         +         '<div class="mdb-sc-skeleton-bar mdb-sc-skeleton-artwork"></div>'
         +     '</div>'
         +     '<div class="mdb-sc-skeleton-row mdb-sc-skeleton-dates">' + bars(3) + '</div>'
         +     '<div class="mdb-sc-skeleton-row mdb-sc-skeleton-buttons">' + bars(3) + '</div>'
         +     '<div class="mdb-sc-skeleton-block mdb-sc-skeleton-toolkit">' + bars(1) + '</div>'
         +     '<div class="mdb-sc-skeleton-block mdb-sc-skeleton-tracklist">' + bars(4) + '</div>'
         + '</div>';
}

// scSkeleton_show
function scSkeleton_show() {
    logFunc( "scSkeleton_show" );

    var wrapper = $("#mdb-sc-trackExtras");

    if( wrapper.length === 0 ) {
        log( "scSkeleton_show: no #mdb-sc-trackExtras - nothing to cover." );
        return;
    }

    // A previous skeleton's timers may still be running (React wiped the wrapper mid-load
    // and the Track header handler recreated it) - they would reveal the new skeleton on
    // the old clock.
    scSkeleton_stop();
    $("#mdb-sc-skeleton").remove();

    // Appended, not prepended: #mdb-sc-trackHead must stay the wrapper's first child for its
    // float (see script.css). While loading the siblings are display:none anyway, and the
    // reveal removes the skeleton in the same step it shows them.
    wrapper.append( scSkeleton_html() ).addClass( "mdb-sc-loading" );

    scSkeleton_shownAt = Date.now();
    scSkeleton_lastMutation = scSkeleton_shownAt;
    ...
    scSkeleton_observer = new MutationObserver(function() {
        scSkeleton_lastMutation = Date.now();
    });
    scSkeleton_observer.observe( wrapper.get(0), { childList: true, subtree: true, attributes: true, characterData: true } );

    scSkeleton_checkTimer = setInterval( scSkeleton_check, 150 );
    scSkeleton_capTimer = setTimeout(function() {
        scSkeleton_reveal( "max wait of " + scSkeleton_maxMs + "ms reached - revealing whatever has arrived" );
    }, scSkeleton_maxMs );

    log( "scSkeleton_show: skeleton up, waiting for buttons + toolkit verdict (settle " + scSkeleton_settleMs + "ms, cap " + scSkeleton_maxMs + "ms)." );
}

// scSkeleton_check
function scSkeleton_check() {
    var wrapper = $("#mdb-sc-trackExtras");

    // SPA navigation or SoundCloud's own re-render removed the wrapper (and the skeleton
    // with it) - stop quietly, the Track header handler starts a fresh skeleton with the
    // recreated wrapper.
    if( wrapper.length === 0 || $("#mdb-sc-skeleton").length === 0 ) {
        log( "scSkeleton_check: wrapper or skeleton gone - stopping timers." );
        scSkeleton_stop();
        return;
    }

    // The failure note (addApiErrorNote, a direct wrapper child) means the buttons will
    // never come - waiting for them would just burn the cap.
    var apiDone = $("#mdb-sc-trackButtons").children().length !== 0
                  || wrapper.children("p.mdb-warning").length !== 0,
        // the exact selector mdbPageCreator_watchToolkit() polls for the verdict
        toolkitDone = $("#mdb-toolkit > ul > li.mdb-toolkit-usageLink.filled").length !== 0,
        quietFor = Date.now() - scSkeleton_lastMutation;

    if( apiDone && toolkitDone && quietFor >= scSkeleton_settleMs ) {
        scSkeleton_reveal( "buttons + toolkit verdict present, wrapper quiet for " + quietFor + "ms" );
    }
}

// scSkeleton_stop
function scSkeleton_stop() {
    if( scSkeleton_checkTimer ) { clearInterval( scSkeleton_checkTimer ); scSkeleton_checkTimer = null; }
    if( scSkeleton_capTimer )   { clearTimeout( scSkeleton_capTimer );    scSkeleton_capTimer = null; }
    if( scSkeleton_observer )   { scSkeleton_observer.disconnect();       scSkeleton_observer = null; }
}

// scSkeleton_reveal
function scSkeleton_reveal( reason ) {
    logFunc( "scSkeleton_reveal" );
    log( "scSkeleton_reveal: " + reason + " (" + ( Date.now() - scSkeleton_shownAt ) + "ms after show)." );

    scSkeleton_stop();

    // Class off and skeleton out in the same synchronous step - no frame ever paints the
    // placeholder and the real content together. mdb-sc-revealed fades the children in.
    $("#mdb-sc-trackExtras").removeClass( "mdb-sc-loading" ).addClass( "mdb-sc-revealed" );
    $("#mdb-sc-skeleton").remove();
}
