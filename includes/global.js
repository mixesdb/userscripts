 ...
 * tidFixFalseCues
 */
function tidMarkFalseCues(tl_arr, maxGap=3) {
  for (let i = 0; i < tl_arr.length; i++) {
    const currentItem = tl_arr[i];
    const prevItem = tl_arr[i - 1];
    const nextItem = tl_arr[i + 1];

    // Prüfen, ob das Item ein "track" ist und "trackText" == "?"
    if (currentItem.type === "track" && currentItem.trackText === "?") {
      const cueDiffPrev = parseInt(currentItem["cue-diff-prev"]);
      const cueDiffNext = nextItem ? parseInt(nextItem["cue-diff-prev"]) : null;

      // Wenn die Differenz zum vorherigen oder zum nachfolgenden Item zu klein ist
      if ((cueDiffPrev < maxGap) || (cueDiffNext && cueDiffNext < maxGap)) {
