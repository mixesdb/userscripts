...
// @downloadRL   https://cdn.rawgit.com/mixesdb/userscripts/refs/heads/main/TrackId.net/script.user.js
 ...
 *
 * ToDos:
 * Embed Hearthis player
 * "Planned features" on Help page
 * Remove all mixesdb and mdb names
 * Fix tracklist textarea layout destroyed by <div contenteditable="plaintext-only">
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
waitForKeyElements(".dashboard", waitMenu);
function waitMenu(jNode) {
    var mixeswikiMenu = '<h3 class="mixeswiki-menu-hl">Requests</h3>';
    mixeswikiMenu += '<ul class="mixeswiki-menu mixeswiki-nolist">';
    mixeswikiMenu += '<li><a href="/submitrequest?from=menu">Submit</a></li>'
    mixeswikiMenu += '<li><a href="/myrequests?from=menu">My requests</a></li>'
    mixeswikiMenu += '</ul>'

    jNode.append( mixeswikiMenu );
}
...
function searchOnMixesWiki(text, target, size) {
    ...
    if (text !== "") {
        ...
        switch (target) {
            case "detail page":
                idName = "mixeswiki-searchLink-detailPage";
                break;
            case "list":
                className = "mixeswiki-searchLink-list";
                break;
        }
...
function funcTidPlayers(jNode, url) {
    ...
    switch (domain) {
        ...
        case "hearthis.at": // https://hearthis.at/toccoscuro/01-manpower-radio1sessentialmix-sat-09-07-2024-talion/
            var embed = '<div class="mixeswiki-info">Sorry, embedding hearthis.at is not possible atm.</div>';
            break;
    }
    //log( embed );

    // embed player
    $(".mixeswiki-player-audiostream").remove();
    if( embed ) {
        jNode.closest(".MuiBox-root").after('<div class="mixeswiki-player-audiostream">' + embed + '</div>');
        jNode.remove();
    }
}
...
/*
 * Tracklist table
 * via table .mixeswiki-tid-table
 */
// waitForKeyElements
waitForKeyElements(".mixeswiki-tid-table:not('.tlEditor-processed')", waitTracklist);
function waitTracklist(jNode) {
    funcTidTracklist(jNode);
}

// funcTidTracklist
function funcTidTracklist(tlWrapper) {
...
function funcTidTables(jNode) {
    ...
    if (grid.length == 1 && grid.is(":visible")) {
        ...
        grid.before('<table class="mixeswiki-tid-table ' + tableClass + ' ' + path + '"><tbody></tbody></table>');
        var tbody = $(".mixeswiki-tid-table tbody");
        ...
        $(".MuiTablePagination-toolbar").insertAfter($(".mixeswiki-tid-table"));
        ...
        if (urlPath(1) == "audiostreams") {
            $(".mixeswiki-tid-table").fadeOut(300); // needs to be visible shortly for tracklist textarea generation
            grid.show();

            // add youtube search icon to grid
        } else {
            grid.addClass('mixeswiki-hide');
        }

        // remove empty th
        //if( !addPlay ) $(".mixeswiki-tid-table tbody th:first-of-type").remove();
    }
...
function on_submitrequest() {
    ...
    if( keywords !== "" ) {
        ...
        function submitRequest_searchInput_wait( jNode ) {
            ...
            var newSearch = '<form id="mixeswiki-replacedSearch" action="https://trackid.net/audiostreams" method="GET">';
